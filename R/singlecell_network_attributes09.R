...
dvh09.original = read.delim("~/Google Drive/Single-Cell-Genomics/variants/singlecell/dvh-09-single-cell-Final-Merged-Variant-Filtered2.txt", sep="\t", header=F)
...
for(row in 1:length(dvh09.original$V1)){
  if("" %in% dvh09.original[row,"V11"]){
    dvh09.original[row,"ID"] = paste("IG", dvh09.original[row, "V2"], sep="_")
  } else {
    dvh09.original[row,"ID"] = sub("DVU_", "DVU", paste(dvh09.original[row, "V11"], dvh09.original[row, "V2"], sep="_"))
  }
}


# read mutations bedfile
dvh09.bedfile = read.delim("/Volumes/omics4tb/sturkarslan/dvh-coculture-rnaseq/dvh-single-cells/dvh-UA3-152-09-singlecell-variants-2callers-80percent-2cells_noan-bed.txt", sep="\t", header=F)
for(row in 1:length(dvh09.bedfile$V1)){
  dvh09.bedfile[row,"ID"] = sub("DVU_", "DVU", paste(dvh09.bedfile[row,"V7"], dvh09.bedfile[row,"V2"], sep="_"))
}
...
# selet only UA3/09 epd line variants
...
dvh09.features = data.frame()
for(name in c(dvh09.names)[[1]]){
  ...
  if(name.us %in% dvh09.original$ID){
    # mutations
    type = as.character(dvh09.original[which(dvh09.original$ID == name.us), "V6"])
    impact = as.character(dvh09.original[which(dvh09.original$ID == name.us), "V7"])
    effect = as.character(dvh09.original[which(dvh09.original$ID == name.us), "V8"])
    # info
    ...
    rownumber = grep(name, dvh09.names$V1)
    in.cells = length(grep(1, dvh09.matrix[rownumber,]))
    notin.cells = length(grep(0, dvh09.matrix[rownumber,]))
    NA.cells = length(grep(3, dvh09.matrix[rownumber,]))
    
  } else {
    cat(name.us, "Not found..\n")
  }
  ## check the clonal isolates file
  if(name %in% dvh09.isolates$ID){
    clones.1 = dvh09.isolates[which(dvh09.isolates$ID == name),"clone"]
  ...
  dvh09.features = rbind(dvh09.features, 
                         cbind(name = name,
